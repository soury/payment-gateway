<?php
namespace PagamentiOnline\GestPay;

if(!function_exists('t')) { function t($str) { return $str; } };

use PagamentiOnline\GestPay\GestPayException;
use PagamentiOnline\GestPay\GestPayCurrency;
use PagamentiOnline\GestPay\GestPayLanguage;

use PagamentiOnline\Classes\Pagamento;

/** Class to manage Banca Sella communication. */
class GestPay extends Pagamento 
{
		...
		foreach(explode(self::SEPARATOR, $decryptedString) as $chunk) {
			...
			if(($equalsPos !== false) && ($equalsPos > 0) && array_key_exists($tagIn = substr($chunk, 0, $equalsPos), $tags)) {
				...
				if(strlen(trim($chunk))) {
					...
					$extra = explode("=", self::decodeFieldValue($chunk));
					$decrypted['customInfo'][$extra[0]] = $extra[1];
				}
			}
		}
		if(empty($decrypted)) {
	...
	*/
	...
	public function createPayment($params=array())
    {
		return [
            ...
            'redirectUrl' => $this->getPaymentURL($this->encrypt($params))
		];
	}
	...
	*/
	...
	public function paymentStatus($params=array())
    {
		...
		$response = $this->decrypt($params['b']);
		$result = [
            'tranId' => $response['transactionID'],
            'bankId' => $response['bankTransactionID'],
